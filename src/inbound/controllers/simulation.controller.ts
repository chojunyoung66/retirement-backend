  ...
  router.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const incomePremium =
          recognizedAnnualIncome > 3_360_000
        ...
        // 건강보험료 합산 (최저보험료 적용)
        const healthPremium = Math.max(
          MIN_HEALTH_PREMIUM,
          incomePremium + propertyPremium + carPremium,
        );
        ...
        const incomeCondition =
          ...
          (businessIncome === 0 || totalAnnualIncome <= 5_000_000);
        const propertyCondition =
          propertyValue <= 540_000_000 &&
          (propertyValue <= 360_000_000 || totalAnnualIncome <= 10_000_000);
  ...
  router.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const { annualContribution, expectedReturnRate, investmentYears } =
          validation.data;
  ...
  router.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const annualTaxCredit = Math.round(
          maxDeductibleContribution * taxCreditRate,
        );
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const incomeTax = Math.round(
          ((deductedIncome * taxRate) / 12) * yearsOfService,
        );
  ...
  router.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const validation = unemploymentBenefitSimulationSchema.safeParse(
          req.body,
        );
        if (!validation.success) {
          const message = validation.error.issues
        ...
        const dailyBenefit = Math.round(
          Math.min(MAX_DAILY, Math.max(MIN_DAILY, dailyWage * 0.6)),
        );
